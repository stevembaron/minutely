...
export function AdminScreen({ onBack, scenario, setScenario, nowMin, setNowMin, onRebuild, location, setLocation, tempOffset, setTempOffset, darkMode = false }: Props) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: bg, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: 'var(--top-safe) 20px 14px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${border}`, background: headerBg }}>
        ...
        <div>
          <div style={{ fontSize: 17, fontWeight: 600, color: text1, letterSpacing: '-0.02em' }}>Admin</div>
          <div style={{ fontSize: 11, color: text3, marginTop: 1 }}>Mock data controls</div>
        </div>
        <div style={{ marginLeft: 'auto', background: devBadgeBg, borderRadius: 20, padding: '3px 10px' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 24px' }}>
          ...
          {(Object.entries(SCENARIOS) as [ScenarioKey, typeof SCENARIOS[ScenarioKey]][]).map(([key, s]) => {
            ...
            return (
              <button key={key} onClick={() => { setScenario(key); onRebuild(key); }} style={{
                ...
                border: `1.5px solid ${active ? (darkMode ? 'rgba(255,255,255,0.2)' : '#1a1a1a') : border}`,
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, color: active ? '#fff' : text1 }}>{s.label}</div>
                  <div style={{ fontSize: 12, color: active ? 'rgba(255,255,255,0.55)' : text3, marginTop: 1 }}>{s.desc}</div>
                </div>
        ...
        <div style={{ background: card, borderRadius: 12, border: `1px solid ${border}`, padding: '16px' }}>
          <input type="range" min="0" max="59" value={nowMin} onChange={e => setNowMin(Number(e.target.value))}
            style={{ width: '100%', accentColor: darkMode ? '#3d9e5f' : '#1a1a1a', cursor: 'pointer' }} />
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
            <span style={{ fontSize: 11, color: text4 }}>0 min</span>
            <span style={{ fontSize: 11, color: text1, fontWeight: 600 }}>+{nowMin} min now</span>
            <span style={{ fontSize: 11, color: text4 }}>59 min</span>
          </div>
        </div>
        ...
        <div style={{ background: card, borderRadius: 12, border: `1px solid ${border}`, padding: '16px' }}>
          <input type="range" min="-20" max="20" value={tempOffset} onChange={e => setTempOffset(Number(e.target.value))}
            style={{ width: '100%', accentColor: darkMode ? '#3d9e5f' : '#1a1a1a', cursor: 'pointer' }} />
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
            <span style={{ fontSize: 11, color: text4 }}>−20°</span>
            <button onClick={() => setTempOffset(0)} style={{ fontSize: 11, color: text3, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Reset</button>
            <span style={{ fontSize: 11, color: text4 }}>+20°</span>
          </div>
        </div>
        ...
        <div style={{ background: card, borderRadius: 12, border: `1px solid ${border}`, overflow: 'hidden' }}>
          {ADMIN_LOCATIONS.map((loc, i) => (
            <div key={loc} onClick={() => setLocation(loc)} style={{
              ...
              <span style={{ fontSize: 14, color: text1, fontWeight: location === loc ? 500 : 400 }}>{loc}</span>
        ...
        <div style={{ background: card, borderRadius: 12, border: `1px solid ${border}`, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[
            ...
            <div key={row.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 13, color: text3 }}>{row.key}</span>
              <span style={{ fontSize: 13, fontWeight: 500, color: text1, fontFamily: 'monospace' }}>{row.val}</span>
            </div>
          ))}
          <button onClick={() => onRebuild(scenario)} style={{
            marginTop: 6, background: rebuildBtnBg, border: 'none', borderRadius: 8,
